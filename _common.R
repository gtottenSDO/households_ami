load_common_packages <- function(pkgs, quietly = TRUE) {
  for (pkg in pkgs) {
    if (!require(pkg, character.only = TRUE, quietly = quietly)) {
      stop(
        sprintf("Package '%s' is required. Install it with install.packages('%s').", pkg, pkg),
        call. = FALSE
      )
    }
  }
...
sdo_allow_writes <- identical(tolower(Sys.getenv("SDO_ALLOW_WRITES", "false")), "true")
sdo_allow_downloads <- identical(tolower(Sys.getenv("SDO_ALLOW_DOWNLOADS", "false")), "true")
