...
acs_mfi_srvy <- fn_create_survey(bind_rows(acs_amis_state, acs_amis_region))

fn_calc_srvy <- function(srvy, hna_region) {
  cost_burden_df <- srvy |>
    filter(region == hna_region) |>
    ...
    mutate(
      ...
      pct_cost_burdened_30 = cost_burdened_30 / total_hh_ten,
      pct_cost_burdened_50 = cost_burdened_50 / total_hh_ten
    )

  return(cost_burden_df)
}

cost_burden_by_ten_ami <- map(
  as.character(0:4),
  \(x) fn_calc_srvy(acs_mfi_srvy, x),
  .progress = TRUE
) |>
  bind_rows()
...
sdo_db_delete_table(
  ...
  schema = "econ",
  table = "hh_by_ten_ami_state_pct"
)

sdo_db_create_table(
  cost_burden_by_ten_ami |> mutate(tenure = str_to_title(paste0(tenure, "er"))),
  con = connect_to_sdo_db(),
  schema = "econ",
