...
mfi_00_04 <- read_csv("MultiYearProfiles0402004.csv") |>
  filter(
    Stub == "Median family income (dollars)" & `Geographic Name` == "Colorado"
  ) |>
...
if (file.exists("data/acs_00_23.rds")) {
  acs_00_23 <- readRDS("data/acs_00_23.rds")
} else {
  ...
  write_rds(acs_00_23, "data/acs_00_23.rds")
}
```
...
| household_type_id/household_type_description | Household type - a factor variable with four levels corresponding to each (id): "One adult with no children" (1), "One adult with children" (2), "More than one adult with no children" (3), "More than one adult with children" (4)- based on characteristics of the household |
| age_group_id/age_group_description | Age group - a factor variable with five levels: "Under 18" (5), "18-24" (1), "25-44" (2), "45-64" (3), "65 and over" (4) - based on the age of the person. |
...
acs_amis <- acs_00_23 |>
  ...
  mutate(
    nadults = sum(zap_labels(AGE) >= 18),
    nchildren = sum(zap_labels(AGE) < 18),
...
ami_percentile_by_year <- acs_amis |>
  ...
  arrange(YEAR, ami_percentile, household_type_id, age_group_id) |>
  left_join(
    acs_mfi |>
      select(YEAR = year, mfi = estimate)
  )
...
The data is then converted to an eviews friendly format for forecsat analysis. This is done at a few different levels, with the most basic being by ami group and tenure, followed by ami group and tenure, and household type. Finally we look at ami group, tenure, household type, and age group.

```{r}
#| label: convert_to_eviews_format

ami_percentile_by_year_eviews <- ami_percentile_by_year |>
  mutate(
    tenure = case_when(
      str_detect(tenure, "Own") ~ "own",
      str_detect(tenure, "Rent") ~ "rent",
      .default = tenure
    )
  ) |>
  select(YEAR, ami_group, tenure, total_hh) |>
  pivot_wider(
    names_from = c(tenure, ami_group),
    values_from = total_hh,
    values_fn = sum
  )
...
write_csv(
  ami_percentile_by_year_eviews,
  "data/ami_percentile_by_year_eviews.csv"
)
```