...
ipums_ddi <- file.path(data_dir, ipums_files[2])
# Load the IPUMS data from RDS file if it exists
if (drive_check_local(ipums_files, data_dir)) {
  acs_00_23 <- read_ipums_ddi(ipums_ddi) |>
    read_ipums_micro()
} else {
  drive_download(ipums_files, data_dir, project_folder = "households_by_ami")
  if (drive_check_local(ipums_files, data_dir)) {
    acs_00_23 <- read_ipums_ddi(ipums_ddi) |>
      read_ipums_micro()
  } else {
