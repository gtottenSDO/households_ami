...
Median Family Income files were downloaded from the HUD website and stored as `acs_mfi` using `r_scripts/read_hud_amis.R`.
```{r}
#| label: acs_mfi
...
acs_mfi_srvy_state <- acs_amis_state |>
  filter(
    PERNUM == 1, # One record per household
    GQ %in% c(1, 2) # Households and group quarters
  ) |>
...
hh_by_ten_ami_state <- acs_mfi_srvy_state |>
...
hh_by_ten_ami_state_pct <- hh_by_ten_ami_state |>
...
cost_burden_by_ami <- acs_mfi_srvy_state |>
...
households_ami_tenure <- acs_srvy |>
  group_by(YEAR, tenure, ami_group) |>
  summarize(
    total_hh = survey_total(),
    .groups = "drop"
...
hh_by_ten_ami_region <- acs_mfi_srvy_region |>
  group_by(YEAR, ami_group, region, tenure) |>
  summarize(
    total_hh = survey_total(),
    .groups = "drop"
  ) |>
  mutate(
    tenure = (str_extract(tenure, "Own|Rent"))
  ) |>
  rename(year = YEAR)
...
write_csv(cost_burden_by_ami, "cost_burden_by_ami.csv")
```
...
DBI::dbWriteTable(
  sdo_con,
  Id(schema = "econ", table = "hh_by_ten_ami_state"),
  hh_by_ten_ami_state_pct,
  overwrite = TRUE
)
...
hh_by_ten_ami_state_eviews <- hh_by_ten_ami_state |>
  mutate(
    ami_group = str_replace(ami_group, "-", "_")
  ) |>
  select(-contains("se")) |>
  pivot_wider(
    names_from = c(tenure, ami_group),
    values_from = hh,
    names_sep = "_"
  )

write_csv(
  hh_by_ten_ami_state_eviews,
  "data/hh_by_ten_ami_state_eviews.csv"
)

rent_own_cost_state <- acs_mfi_srvy_state |>
  group_by(YEAR, tenure) |>
  summarize(cost = survey_quantile(housing_cost, c(0.25, .5, 0.75))) |>
  pivot_longer(
    cols = starts_with("cost"),
    names_to = "cost"
  ) |>
  unite("var", tenure, cost, sep = "_") |>
  pivot_wider(names_from = var, values_from = value)

write_csv(
  rent_own_cost_state,
  "data/acs_rent_own.csv"
)

write_csv(
  mfi_state |>
    rename(mfi_state = mfi),
  "data/acs_mfi_state.csv"
)

drive_folder = as_id("1TdbwsSvZXVOeDAZUhvl2IF4wAlFr-Qpb")
drive_put(
  "data/hh_by_ten_ami_state_eviews.csv",
  path = drive_folder
)
...
DBI::dbWriteTable(
  connect_to_sdo_db(),
  Id(schema = "econ", table = "hh_by_ten_ami_region"),
  hh_by_ten_ami_region,
  overwrite = TRUE
)

write_csv(
  hh_by_ten_ami_region,
  "data/hh_by_ten_ami_region.csv"
)

drive_put(
  "data/hh_by_ten_ami_region.csv",
  path = as_id("1sUrHAmHzHFgX0Pbfh--2DhD26t8tn0JP")
)
```