...
acs_amis <- as_duckplyr_df(acs_00_23) |>
  ...
  mutate(
    ...
    ami_percentile = ceiling(pct_mfi*100) / 100 ,
    cost_burdened_30 = case_when(
      OWNCOST*12/HHINCOME >= .3 ~ TRUE,
      .default = FALSE),
    cost_burdened_50 = case_when(
      OWNCOST*12/HHINCOME >= .5 ~ TRUE,
      .default = FALSE),
...
ami_percentile_by_year <- acs_amis |>
  ...
  summarize(total_hh = sum(HHWT), .by = c(YEAR, household_type_id, ami_percentile, age_group, cost_burdened_30, cost_burdened_50, OWNERSHP, OWNERSHPD)) |>
